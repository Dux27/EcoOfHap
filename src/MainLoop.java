public class MainLoop {
    ...
    public static void startGame(UI uiInstance) {
        ui = uiInstance; 
        if (running) 
            ...
            while (running) {
                ...
                if (TIC_COUNTER % 12 == 0 && TIC_COUNTER != 0 && !yearUpdatePrinted) {
                    ...
                    for (Item item : ui.player.inventory) {
                        if ("Houses".equals(item.category)) {
                            item.increaseValue(0.02f); // Increase value by 2%
                            System.out.println("House values updated for year: " + (TIC_COUNTER / 12) + " - " + item.getName() + ": $" + item.sellPrice);
                        }
                    }
                    ui.player.age++;
